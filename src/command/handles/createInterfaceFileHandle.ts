...
import { generateUrl, getFolderPath, checkConfig, getHttpBody, handleJsonSchema } from "../../utils"
...
class CreateInterfaceFile {
    ...
    async getServerData() {
        ...
        if (Array.isArray(resp2.data.list)) {
            for (let index = 0; index < resp2.data.list.length; index++) {
                ...
                const resp3: any = await getHttpBody(this.yapiConfig.baseUrl + "/api/interface/get", {
                    token: this.yapiConfig.token,
                    id: item._id
                })
    ...
    // 处理json_schema
    async resolveinJsonSchema(data: string, interfaceName: string) {
